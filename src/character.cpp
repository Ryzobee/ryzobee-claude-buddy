...
// Draw target — defaults to the sprite's backing device; characterRenderTo
// can retarget to a display device for landscape clock.
...
static void gifPlace() {
  ...
  if (_sprite) gifX = (_sprite->width() - outW) / 2;
...
static void gifDrawCb(GIFDRAW* d) {
  ...
  auto put = [&](int x, int y, uint8_t idx) {
    ...
    if (_direct) _direct->drawPixel(x, y, color);
    else if (_sprite) _sprite->drawPixel(x, y, color);
    else if (_lcd) _lcd->drawPixel(x, y, color);
  };
  ...
  int y = gifY + srcY;
  if (!_sprite) return;
  if (y < 0 || y >= _sprite->height()) return;
  int x0 = gifX + d->iX;
  int w  = d->iWidth;
  if (w > 256) w = 256;
  if (x0 < 0) { src -= x0; w += x0; x0 = 0; }
  if (x0 + w > _sprite->width()) w = _sprite->width() - x0;
  if (w <= 0) return;
...
bool characterInit(const char* name) {
  ...
  // Ensure drawing targets are set by caller via buddyInit() or main
  if (!_lcd && _sprite == nullptr) {
    // main typically sets these after board init; leave as-is if not provided
  }
  return true;
}
...
void characterSetTarget(lgfx::LGFX_Device* lcd, lgfx::LGFX_Sprite* sprite) {
  _lcd = lcd;
  _sprite = sprite;
  if (gifW > 0 && gifH > 0) gifPlace();
}

// One-shot half-scale render to an arbitrary LovyanGFX surface. Caller owns
// clearing. Advances frame timing so animation runs even when characterTick()
// is bypassed.
void characterRenderTo(lgfx::LGFX_Device* tgt, int cx, int cy) {
  ...
  lgfx::LGFX_Device* prevT = _direct; bool prevP = peekMode; int px = gifX, py = gifY;
  _direct = tgt; peekMode = true;
  gifX = cx - gifW / 4;
  gifY = cy - gifH / 4;
  uint32_t now = millis();
  if (now >= nextFrameAt) {
  ...
  _direct = prevT; peekMode = prevP; gifX = px; gifY = py;
}
...
void characterInvalidate() {
  if (!loaded) return;
  if (textMode) {
    if (_sprite) _sprite->fillSprite(pal.bg);
...
void characterSetState(uint8_t s) {
  ...
  if (gif.open(full, gifOpenCb, gifCloseCb, gifReadCb, gifSeekCb, gifDrawCb)) {
    ...
    gifPlace();
    if (_sprite) _sprite->fillSprite(pal.bg);   // bias upward, leave room for HUD
    nextFrameAt = 0;
...
void characterTick() {
  ...
  if (textMode) {
    ...
    if (_sprite) _sprite->fillRect(0, cy - 14, _sprite->width(), 28, pal.bg);

    const char* line = ts.frames[textFrame];
    int len = strlen(line);
    int tw = len * 12;                                    // size-2 glyph width
    if (_sprite) {
      _sprite->setTextColor(pal.body, pal.bg);
      _sprite->setTextSize(2);
      _sprite->setCursor((_sprite->width() - tw) / 2, cy - 8);
      _sprite->print(line);
    }
