...
// Render target indirection: defaults to the sprite's backing LCD device.
...
static lgfx::LGFX_Device* _direct = nullptr;
...
void buddyPrintLine(const char* line, int yPx, uint16_t color, int xOff) {
  ...
  if (_direct) {
    _direct->setTextColor(color, BUDDY_BG);
    _direct->setCursor(x, yPx);
    for (int i = 0; i < len; i++) _direct->print(line[i]);
  } else if (_sprite) {
    _sprite->setTextColor(color, BUDDY_BG);
    _sprite->setCursor(x, yPx);
    for (int i = 0; i < len; i++) _sprite->print(line[i]);
  } else if (_lcd) {
    _lcd->setTextColor(color, BUDDY_BG);
    _lcd->setCursor(x, yPx);
    for (int i = 0; i < len; i++) _lcd->print(line[i]);
  }
}

void buddyPrintSprite(const char* const* lines, uint8_t nLines, int yOffset, uint16_t color, int xOff) {
  if (_direct) _direct->setTextSize(_scale);
  else if (_sprite) _sprite->setTextSize(_scale);
  else if (_lcd) _lcd->setTextSize(_scale);
...
void buddySetCursor(int x, int y) {
  int tx = BUDDY_X_CENTER + (x - BUDDY_X_CENTER) * _scale;
  int ty = y * _scale;
  if (_direct) _direct->setCursor(tx, ty);
  else if (_sprite) _sprite->setCursor(tx, ty);
  else if (_lcd) _lcd->setCursor(tx, ty);
}
void buddySetColor(uint16_t fg) {
  if (_direct) _direct->setTextColor(fg, BUDDY_BG);
  else if (_sprite) _sprite->setTextColor(fg, BUDDY_BG);
  else if (_lcd) _lcd->setTextColor(fg, BUDDY_BG);
}
void buddyPrint(const char* s) {
  if (_direct) { _direct->setTextSize(_scale); _direct->print(s); }
  else if (_sprite) { _sprite->setTextSize(_scale); _sprite->print(s); }
  else if (_lcd) { _lcd->setTextSize(_scale); _lcd->print(s); }
}
...
// One-shot render to an arbitrary LGFX surface (landscape clock).
// Bypasses tick gating and the sprite fillRect — caller owns
...
void buddyTick(uint8_t personaState) {
  ...
  if (_sprite) {
    _sprite->fillRect(0, 0, BUDDY_CANVAS_W,
               (BUDDY_Y_BASE + 5 * BUDDY_CHAR_H + 12) * _scale, BUDDY_BG);
  } else if (_lcd) {
    _lcd->fillRect(0, 0, BUDDY_CANVAS_W,
               (BUDDY_Y_BASE + 5 * BUDDY_CHAR_H + 12) * _scale, BUDDY_BG);
  }
