...
// Set true once the bridge sends a time sync. Root Maker currently has no
// RTC API in the BSP, so this only records that the host supplied time.
static bool _timeSynced = false;
inline bool dataRtcValid() { return _timeSynced; }
