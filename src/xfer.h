...
// Ack goes to both streams. We do not track which one delivered the command,
// and writes to a disconnected BLE client are dropped. The bridge listens on
...
inline bool xferCommand(JsonDocument& doc) {
  ...
  if (strcmp(cmd, "status") == 0) {
    // Dump everything the info screens show. Manual printf has less heap
    // churn here than serializing a temporary ArduinoJson document.
    // Root Maker battery support is not exposed by the current BSP yet, so
    // return placeholder values here.
    // TODO: replace with rootmaker.bat.* when available.
    int vBat = 3700;
    int iBat = 0;
    int vBus = 0;
  ...
  if (strcmp(cmd, "char_begin") == 0) {
    ...
    // Headroom for LittleFS metadata overhead; it is not byte-for-byte.
